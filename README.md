...
This page features a short description of the program and outlines its usage and architecture while also instructing how to generate the doxygen API documentation. License and team information are found at the bottom.
...
Nodes serve as platforms for connections and applications, which can be stacked into the somewhat freely. These applications have preset properties which in sequential order execute the handling of incoming and outgoing packets. They are labeled with distinct labels, which simultaneously serve as their addresses.

Packets are very basic packets and are generated by a specific kind of an application, the PacketGenerator. 

XML is used to define specific network layouts for the programs. The relative link should be given as an argument when launching the program. Read 'Usage' part for detailed instructions for how to write XML configuration file in correct form. Few test layouts have been provided for use in the 'resources' folder.
